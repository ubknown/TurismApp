...
const EditUnitPage = () => {
  ...
  const fetchUnitDetails = async () => {
    try {
      ...
      const response = await api.get(`/api/units/${id}/owner-details`);
      const unit = response.data;
      
      // Parse location back to county and address for backward compatibility
      const locationParts = unit.location ? unit.location.split(', ') : ['', ''];
      const address = locationParts.slice(0, -1).join(', ');
      const fallbackCounty = locationParts[locationParts.length - 1];
      
      setFormData({
        name: unit.name || '',
        description: unit.description || '',
        county: unit.county || fallbackCounty || '', // Use dedicated county field first, fallback to parsing location
        address: address || '',
        capacity: unit.capacity?.toString() || '',
        pricePerNight: unit.pricePerNight?.toString() || '',
      ...
      if (error.response?.status === 403) {
        ...
        showError('Unit Not Found', 'The requested unit could not be found');
        navigate('/my-units');
      } else {
        showError('Load Error', 'Failed to load unit details');
      }
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (error.response?.status === 400) {
        ...
        errorMessage = 'You do not have permission to edit this unit';
      } else if (error.response?.status === 401) {
        errorMessage = 'Please log in to edit units';
        navigate('/login');
        return;
      } else if (error.response?.status === 404) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-white">Loading unit details...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen p-6">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h1 className="text-4xl font-bold text-white mb-2">Edit Unit</h1>
            <p className="text-violet-200">Update your accommodation unit details.</p>
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <select
                ...
                onChange={handleInputChange}
                className="w-full px-4 py-3 bg-white/5 backdrop-blur-md border border-white/20 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-violet-500/50 focus:border-violet-500/50 transition-all duration-300"
                disabled={loading}
              >
